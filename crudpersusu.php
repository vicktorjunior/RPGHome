...
if(isset($nome_personagem) && $acao == 'insercao'){
   // if($acao != 'atualizarFim') {
    ...
    $string = "SELECT id_classe from classes where nome_classe LIKE '$nome_classe'";
    $pesq = $db->queryAsArray($string);
    //echo $pesq;
    if (!$pesq) {
        $dadoClasse = array($nome_classe,'');
        $op = $db->execute("INSERT INTO classes (nome_classe,historia) values (?,?)",$dadoClasse);
        if (!$op) {
            $db->rollback();
        }

        $classe = $db->lastInsertId('classes_id_classe_seq');
        //echo "$classe";
       // die("errou");
    } else {
        foreach ($pesq as $linha) {
            $classe = $linha['id_classe'];
            echo $classe;
        }

    }
    $dados = array($nome_personagem,$classe,$nivel,$exp,$defesa,$stamina,$vida,$mana,$forca,$agilidade,$inteligencia,$vontade,$iduser);
    $op = $db->execute("INSERT INTO personagens 
                              (nome_personagem,
                              id_classe,
                              nivel,
                              experiencia,
...
if(isset($id) && $acao == 'deletar'){
    $dados2[0] = $id;
    $db->begin();
    $op = $db->execute("DELETE FROM personagens WHERE id_personagem=?",$dados2);
    if (!$op) {
        $db->rollback();
    } else {
        $db->commit();
...
if(isset($acao) && $acao == 'atualizarFim'){
    ...
    $id=null;
    //echo "atualizar fim";
    $op = $db->execute("UPDATE personagens SET nome_personagem=?, 
                                         id_classe=?,
                                         nivel=?, 
                                         experiencia=?, 
                                         defesa=?, 
...
if(isset($acao) && $acao == 'atualizar'){
    ...
    //$pesqclasse = $db->query("SELECT nome_classe FROM classes WHERE nome_classe LIKE '$nome_classe'");
    //foreach ($pesqclasse as $linha) {
    //    $dadosTemp['nome_classe'] = $linha;
    //}
...
<div class="container-fluid">
    ...
    <div>
        <div class="panel panel-default panel-table">
                    ...
                    <form role="form" action="crudpersusu.php?acao=<?= $acao?>&id=<?=$id?>"" method="post">
                        ...
                        <div class="row"><div class="col-md-6">Nome do Personagem: <input type="text" required name="nome_personagem" value="<?=$dadosTemp['nome_personagem']?>"></div>
                            <div class="col-md-6">Classe:
                                <input type="text" name="nome_classe" required value="<?=$dadosTemp['nome_classe']?>">
                            </div>
                        </div>
